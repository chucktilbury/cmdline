    ...
    if(cmds->cfirst != NULL) {
        for(crntc = cmds->cfirst; crntc != NULL; crntc = nextc) {
            ...
            if(crntc->type == CL_STRING)
                free_mem((void*)crntc->data.str);
            free_mem((void*)crntc->name);
            free_mem((void*)crntc->param);
            free_mem((void*)crntc->help);
            free_mem(crntc);
        }
    }
...
void parseCmdLine(int argc, char** argv)
{
    ...
    cmds->fname = dup_string(argv[index++]);
...
static inline _cmd_line_elem* create_cel(CLType type, const char* param, const char* name, const char* help)
{
    ...
    cel->name = dup_string(name);
    cel->help = dup_string(help);
    cel->type = type;
...
void addCBwParam(const char* param, const char* name, const char* help, callback_w def, CLFlags flags)
{
    _cmd_line_elem* cel = create_cel(CL_CB_W, param, name, help);
    cel->data.with = def;
    cel->flags = flags;
...
void addCBwoParam(const char* param, const char* name, const char* help, callback_wo def, CLFlags flags)
{
    _cmd_line_elem* cel = create_cel(CL_CB_WO, param, name, help);
